// ==========================================================
// بوت تليجرام المطور للتحكم بسيرفرات HopX (النسخة الاحترافية الفائقة v3.1.0)
// نظام الحماية والتحمل الفائق:
//   1) دمج الـ Keep-Alive التلقائي (كل 4 دقائق) لمنع حذف السيرفر.
//   2) ميزة الكشف عن الأوامر الانتحارية والخطيرة لحماية البيئة السحابية.
//   3) نظام التنظيف الدوري للذاكرة العشوائية RAM وملفات الكاش (كل ساعتين).
//   4) أداة اقتطاع النصوص وحماية البوت من الانهيار عند قراءة السجلات الضخمة.
//   5) سجل السيرفرات النشطة التفاعلي مع أزرار استعادة الجلسة الفورية.
//   6) تتبع دقيق لمعرفات العمليات (PIDs) والتشغيل الخلفي الآمن (setsid).
// ==========================================================

console.log("\n==================================================");
console.log(`🚀 [STAMP] تم تشغيل النسخة الاحترافية الفائقة v3.1.0 بنجاح!`);
...
const RECONNECT_RETRIES = 3;                        // عدد محاولات الاتصال بالسيرفر قبل إعلان فشله
...
console.log('🚀 البوت المطور v3.1.0 مستعد ومحمي بالكامل...');
...
// تفعيل نبضات منع الحذف التلقائي خمولاً
...
async function getActiveSandbox(chatId) {
  ...
  for (let attempt = 1; attempt <= RECONNECT_RETRIES; attempt++) {
    try {
      currentSandbox = await withTimeout(
        Sandbox.connect({ sandboxId: appState.sandboxId, apiKey: appState.apiKey }),
        20000,
        `إعادة الاتصال المحاولة رقم ${attempt}`
      );
      console.log(`✅ تم استعادة قنوات الاتصال بالسيرفر المحفوظ بنجاح: ${appState.sandboxId}`);
      startKeepAlive();
  ...
  // فشل الإنعاش بعد كل المحاولات: أرشفة السيرفر وحذفه من النشط حالياً
  ...
  if (chatId) {
    await bot.sendMessage(
      chatId,
      `⚠️ تعذر الاتصال التلقائي بالسيرفر الحالي بعد ${RECONNECT_RETRIES} محاولات.\n\nالسبب الأخير: ${lastError ? lastError.message : 'انتهاء الجلسة أو الحذف التلقائي'}\n\nيمكنك الانتقال إلى قسم "سيرفراتي السابقة" لإعادة الاتصال اليدوي أو إنشاء سيرفر جديد ومستقل.`,
      getStartKeyboard()
    );
  }
  return null;
